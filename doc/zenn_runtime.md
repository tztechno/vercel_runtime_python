...
フロントでPython関数を定義し、データを入力して計算を実行させると、結果が表示されます。
```
...
## 終わりに
計算式をフロントから提供してバックエンドで動かすことができるようになりました。
AtCoderのカスタムコードテストと似たようなものになりました。
計算結果の正誤判定はできませんが、計算式の計算速度に与える影響などの検討には使えるかと思います。
Vercelにアップしています。
